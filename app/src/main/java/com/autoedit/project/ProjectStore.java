...
public class ProjectStore {
    ...
    public void save(EditProject p){
        try{ ctx.getSharedPreferences(PREF,0).edit().putString(KEY,toJsonString(p)).apply(); }catch(Exception ignored){}
    }
    ...
    public EditProject fromJsonString(String s){
        EditProject p=new EditProject(); if(s==null) return p;
        try{
            JSONObject o=new JSONObject(s); p.name=o.optString("name",p.name); p.fps=o.optInt("fps",30); p.width=o.optInt("width",1080); p.height=o.optInt("height",1920);
