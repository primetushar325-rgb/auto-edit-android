...
public class VideoExporter {
    ...
    public void export(EditProject project, ExportOptions opts, Listener listener) throws Exception {
        File out = new File(opts.outputPath); File parent = out.getParentFile(); if(parent!=null) parent.mkdirs();
        ...
        try {
            ...
            muxer = new MediaMuxer(opts.outputPath, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
            ...
            for(int ci=0; ci<project.clips.size(); ci++){
                ...
                for(int f=0; f<frames; f++){
                    ...
                    frameIndex++; if(listener!=null && frameIndex%Math.max(1,opts.fps/2)==0){ ExportProgress p=new ExportProgress(); p.currentClip=ci+1; p.currentFrame=frameIndex; p.totalFrames=total; p.percent=(int)Math.min(99, frameIndex*100/Math.max(1,total)); p.message="Rendering clip "+(ci+1)+" / "+project.clips.size(); listener.onProgress(p); }
                }
            }
        ...
        } catch(Exception e){ Log.e(TAG,"Export failed",e); if(out.exists()) //noinspection ResultOfMethodCallIgnored
                out.delete(); throw e; }
